...
/*	public void fillContent(ACLMessage msg, AbsContentElement content) throws CodecException, OntologyException {
	...
	public void fillContent(AclMessage msg, AbsContentElement content) throws CodecException, OntologyException {
		Codec    codec = lookupLanguage(msg.getAclPayload().getLanguage());
		if (codec == null) {
			throw new CodecException("Unknown language "+msg.getAclPayload().getLanguage());
		}
		String ontoName = msg.getAclPayload().getOntology();
		Ontology o = null;
		if (ontoName != null) {
			o = lookupOntology(ontoName);
			if (o == null) {
				throw new OntologyException("Unknown ontology "+msg.getAclPayload().getOntology());
			}
		}
		Ontology onto  = getMergedOntology(codec, o);
...
/*	public AbsContentElement extractAbsContent(AclMessage msg) throws CodecException, OntologyException {
	...
	public Ontology getOntology(AclMessage msg) {
		return getMergedOntology(lookupLanguage(msg.getAclPayload().getLanguage()), lookupOntology(msg.getAclPayload().getOntology()));
	}
	//#APIDOC_EXCLUDE_END
...
/*	private void encode(ACLMessage msg, AbsContentElement content, Codec codec, Ontology onto) throws CodecException, OntologyException { 
	...
	private void encode(AclMessage msg, AbsContentElement content, Codec codec, Ontology onto) throws CodecException, OntologyException { 
		if (codec instanceof StringCodec)
			msg.getAclPayload().setContent(((StringCodec) codec).encode(onto, content));
		else
			throw new CodecException("UnsupportedTypeOfCodec");
	}
...
/*	private AbsContentElement decode(ACLMessage msg, Codec codec, Ontology onto) throws CodecException, OntologyException { 
	...
	private AbsContentElement decode(AclMessage msg, Codec codec, Ontology onto) throws CodecException, OntologyException { 
		if (codec instanceof StringCodec)
			return ((StringCodec) codec).decode(onto, msg.getAclPayload().getContent());
		else
			throw new CodecException("UnsupportedTypeOfCodec");
	}
